...
from pathlib import Path
    ...
    $line = # main.py — GlassServer (ASCII only)
import os
import sqlite3
import time
import secrets
...
# ===== App =====
app = 
  $args = # main.py — GlassServer (ASCII only)
import os
import sqlite3
import time
import secrets
...
.Groups[1].Value
  if ($args -match 'redirect_slashes') { "FastAPI($args)" }
  elseif ($args -match '\S') { "FastAPI($args, redirect_slashes=False)" }
  else { "FastAPI(redirect_slashes=False)" }
...
.Groups[1].Value
    if ($line -notmatch '\bResponse\b') {
        "from fastapi import $line, Response"
    } else {
        "from fastapi import $line"
    }
